...
import {
  validateCardanoBalanceRequest,
  validateAssetsRequest,
  validateCardanoPollRequest,
} from './cardano.validators';
...
export class CardanoController {
  static async poll(
    ...
    try {
      transactionDetails = await cardano.getTransaction(req.txHash);
    } catch (error) {
  ...
  static async getTokens(
    ...
    cardanoTokens.map((cardanoToken) => {
      const token: TokenInfo = {
        ...
        decimals: cardanoToken.decimals,
